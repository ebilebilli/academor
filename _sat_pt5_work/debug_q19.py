...
loc = m.find_marker_on_pages(doc, pages["math1"], pages["math1_end"], 19)
pidx, marker = loc
page = doc[pidx]
...
out = io.open("_sat_pt5_work/dbg_q19.txt", "w", encoding="utf-8")
for L in "ABCD":
    r = bounds[L]
    ...
    for block in d.get("blocks", []):
        ...
        for line in block.get("lines", []):
            for span in line.get("spans", []):
                out.write(f"  RAW span {span['text']!r} bbox={[round(x,2) for x in span['bbox']]} flags={span['flags']} font={span.get('font','')}\n")
out.close()
print("done")
