...
def create_courseenrollment_if_missing(apps, schema_editor):
    with schema_editor.connection.cursor() as cursor:
        cursor.execute(
            ...
            WHERE table_schema = CURRENT_SCHEMA()
              AND table_name = 'payments_courseenrollment'
            """
        )
        if cursor.fetchone():
            return
    CourseEnrollment = apps.get_model('payments', 'CourseEnrollment')
    schema_editor.create_model(CourseEnrollment)
        ...
        migrations.SeparateDatabaseAndState(
            ...
            database_operations=[
                migrations.RunPython(
                    create_courseenrollment_if_missing,
