...
def invalidate_model_cache(model_name):
    ...
    Service (e.g. admin ``card_icon`` or mock test flags) uses the same bump — see
    ``projects.signals.invalidate_service_cache``.

    Tagline changes bump all versioned keys (inner-page banner taglines).
    CoursePricePackage changes (incl. mock ``credits``) bump via
    ``invalidate_course_price_package_cache`` (Service).
    """
    _bump_cache_version()
