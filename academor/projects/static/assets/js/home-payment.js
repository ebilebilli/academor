(function () {
    ...
    function boot() {
        var payModalEl = document.getElementById("homeCoursePayModal");
        if (!payModalEl || payModalEl.dataset.homePayInit === "1") {
            return;
        }
        payModalEl.dataset.homePayInit = "1";

        if (payModalEl.parentElement !== document.body) {
            document.body.appendChild(payModalEl);
        }

        var form = document.getElementById("home-course-pay-form");
        var modalAlertEl = document.getElementById("home-course-pay-modal-alert");
        var modalPackageEl = document.getElementById("home-course-pay-modal-package");
        var submitBtn = document.getElementById("home-course-pay-submit");
        ...
        var contractBodyEl = document.getElementById("home-course-pay-contract-body");
        var contractPanelEl = document.getElementById("home-course-pay-contract-panel");
        var contractTemplatesEl = document.getElementById("home-pay-contract-templates");
        var submitLabel =
        ...
        function setFieldError(fieldName, message) {
            ...
            if (!input) {
                input = form.querySelector('[name="home-' + fieldName + '"]');
            }
            if (!input) {
                return;
            }
        ...
        document.addEventListener("click", function (e) {
            var btn = e.target.closest("[data-home-pay-open]");
            if (!btn) {
                return;
            }
            e.preventDefault();
        ...
        if (payModalEl.getAttribute("data-open-on-load") === "true") {
            ...
            var trigger = initialId
                ? document.querySelector(
                      '[data-home-pay-open][data-package-id="' + initialId + '"]'
                  )
                : document.querySelector("[data-home-pay-open]");
            if (trigger) {
